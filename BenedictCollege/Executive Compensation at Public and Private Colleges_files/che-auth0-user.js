...
var checkPageIntegrity = function() {
  ...
  const CPIdisplaySLAccessExpired = function() {
    const CPItitle = atob("WW91ciBjaHJvbmljbGUuY29tIGFjY2VzcyBoYXMgZXhwaXJlZC4=");
    const CPItext = atob("UGxlYXNlIHZpc2l0IDxhIGhyZWY9Imh0dHBzOi8vd3d3LmNocm9uaWNsZS5jb20iPmNocm9uaWNsZS5jb208L2E+IG9yIGNvbnRhY3QgeW91ciBhZG1pbmlzdHJhdG9yIGZvciBmdXJ0aGVyIGRldGFpbHMgcmVnYXJkaW5nIHJlbmV3YWwgb2YgaW5zdGl0dXRpb25hbCBhY2Nlc3Mu");
    CPIEventDispatch('CPIExpiredMsgShown');
    logMessage('Showing Access Expired Message');
    CPIdisplayModal(CPItitle, CPItext, false);
  }

  const CPIdisplayImportantMessage = function() {
    const CPItitle = atob("QWQgQmxvY2tlciBEZXRlY3RlZA==");
    const CPItext = atob("UGxlYXNlIHN1cHBvcnQgb3VyIHNpdGUgYnkgd2hpdGVsaXN0aW5nIGNocm9uaWNsZS5jb20uIFdlIGRlcGVuZCBvbiBhZHZlcnRpc2luZyBpbiBvcmRlciB0byBwcm92aWRlIHlvdSB3aXRoIGVzc2VudGlhbCBpbmRlcGVuZGVudCBqb3VybmFsaXNtLg==");
    CPIEventDispatch('CPIWhitelistMsgShown');
    logMessage('Showing Whitelist Message');
    CPIdisplayModal(CPItitle, CPItext, true);
  }
      ...
      if (digitalData) {
        if (digitalData.page && digitalData.page.category && digitalData.page.category.pageType) {
          ...
          for (let i = 0; i < pageTypeList.length; i++) {
              if (pageType.indexOf(pageTypeList[i]) > -1) {
  ...
  const checkForServices = function() {
    let blueConicDetected = true;
    let blueConicBypass = false;
    try {
      ...
      if (window.blueConicClient) {
        if (window.blueConicClient.isInSimulatorMode()) {
          logMessage('BlueConic in simulator mode');
          blueConicBypass = true;
        }
        if (window.blueConicClient.isInEditMode()) {
          logMessage('BlueConic in edit mode');
          blueConicBypass = true;
        }
      }
    ...
    if (blueConicBypass) {
      logMessage('BlueConic bypass mode');
      return true;
    }
...
if (!window.jQuery) {
  (function() {
    ...
    script.onload = function() {
      ...
      $(document).ready(function() {
        try {
          setTimeout(function(){checkPageIntegrity();},8000);
        } catch (e) {
