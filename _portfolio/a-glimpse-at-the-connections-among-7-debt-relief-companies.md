---
title: A Glimpse at the Connections Among 7 Debt-Relief Companies
image:
  path: "/assets/media/pexels-pixabay-268949.webp"
  caption: Pixabay via Pexels
  sourceurl: https://www.pexels.com/photo/spider-web-with-water-drops-268949/
categories:
- portfolio
date: '2014-08-04 00:01:00'
link:
  text:
  url:
tags: 
- text-mining
- business-reporting
- collaboration
- developer-tools
- higher-ed-reporting
- investigative-projects
- legal-reporting
- news-writing-editing
- personal-finance-reporting
- project-management
- consumer-protection-filings
- federal-state-court-records
- metadata-sleuthing
- us-uk-canada-business-records
- webpage-monitoring
layout: portfolio_wide_paywall
author: Dan Bauman
republished:
  pubname: The Chronicle of Higher Education
  puburl: https://www.chronicle.com/article/a-glimpse-at-the-connections-among-7-debt-relief-companies/
  reason: Paywall; Degraded Assets
  archiveurl: "/assets/media/archive/a-glimpse-at-the-connections-among-7-debt-relief-companies.pdf"
---

<p style="color:black">It can be hard to divine who is behind debt-relief websites, in part because the relationships between the companies can be complex. Using information from legal orders, domain registrations, and state-incorporation documents, <em>The Chronicle</em> created this chart to visualize the connections among four individuals and the companies with which they have been affiliated.</p> 

<p style="color:black">With the closing of Student Loan Solutions LLC in 2008, six companies emerged, focused on student-loan relief services. Mouse over the chart to find details on the companies and links that document their relationships.</p>

<p style="color:black"><em>View the related story: <strong><a class="Link" href="/portfolio/the-pitch-debt-relief-the-reality-borrowers-may-pay-more/" target="_blank" rel="noopener" data-cms-ai="0">The Pitch: ‘Debt Relief.’ The Reality: Borrowers May Pay More.</a></strong></em></p><br>

<div style="width: 100%;"><div style="position: relative; padding-bottom: 129%; padding-top: 0; height: 0;"><iframe title="Info" frameborder="0" width="600" height="774" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%;" src="https://view.genially.com/6977060249b19f75c2538083" type="text/html" allowscriptaccess="always" allowfullscreen="true" scrolling="yes" allownetworking="all"></iframe> </div> </div>